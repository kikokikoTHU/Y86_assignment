...
instr_t instruction_set[] = 
{
    ...
    /* Ex.2 */
    {"mutex",  HPACK(I_MUTEX, F_NONE), 6, M_ARG, 1, 0, R_ARG, 1, 1 },
...
void init_sharemem() {
    ...
    if (shm_addr == NULL) {
        shm_id = shmget(SHMKEY, SHMEND - SHMSTART, 0666|IPC_CREAT);
        if (shm_id == -1) {
            perror("shmget error");
        } else {
            shm_addr = shmat(shm_id, NULL, 0);
            shm_size = SHMEND - SHMSTART;
        }
    }
...
mem_t copy_mem(mem_t oldm)
{
    ...
    {
        if(i >= SHMSTART && i < SHMEND)
        {
            newm->contents[i] = shm_addr[i-SHMSTART];
        }
        else
        {
...
bool_t diff_mem(mem_t oldm, mem_t newm, FILE *outfile)
{
    ...
    for (pos = 0; (!diff || outfile) && pos < len; pos += 4) {
    	...
    	get_word_val_primem(oldm, pos, &ov);
...
bool_t get_byte_val(mem_t m, word_t pos, byte_t *dest)
{
    ...
    /* Ex.2 */
    if (pos >= SHMSTART && pos < SHMEND) {
        *dest = shm_addr[pos - SHMSTART];
        return TRUE;
    }
...
bool_t get_word_val(mem_t m, word_t pos, word_t *dest)
{
    ...
    if (pos >= SHMSTART && pos < SHMEND) {
        for (i = 0; i < 4; i++) {
            val = val | shm_addr[pos - SHMSTART + i]<<(8*i);
        }
        *dest = val;
...
bool_t set_word_val(mem_t m, word_t pos, word_t val)
{
    ...
    if (pos >= SHMSTART && pos < SHMEND) {
        for(i = 0; i < 4; i++)
        {
            shm_addr[pos - SHMSTART + i] = val & 0xFF;
            val >>= 8;
        }
        return TRUE;
    }
...
/* Ex.2 */
/* The function for retrieving the val without considering shared memory */
bool_t get_word_val_primem(mem_t m, word_t pos, word_t *dest)
{
    int i;
    word_t val;
    if (pos < 0 || pos + 4 > m->len)
...
bool_t diff_reg(mem_t oldr, mem_t newr, FILE *outfile)
{
    ...
    for (pos = 0; (!diff || outfile) && pos < len; pos += 4) {
        word_t ov = 0;
        word_t nv = 0;
	get_word_val_primem(oldr, pos, &ov);
